...
| Quantization | fp8 |
| `local_dir` (under `MODEL_CACHE_ROOT`) | `qwen36-27b-fp8` |

## Fetch the weights (one-time, per host)

The adapter expects the snapshot at `$MODEL_CACHE_ROOT/qwen36-27b-fp8` and mounts
`$MODEL_CACHE_ROOT` into the container at `/hf-cache` read-only. Fetch the pinned
revision there explicitly — do **not** rely on Hugging Face's internal
snapshot-cache layout, which can move:
