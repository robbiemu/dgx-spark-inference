...
# See bundles/experimental/laguna-s-2.1-nvfp4-dflash/README.md.
#
# Safety (load-bearing):
...
EXP_IMAGE="${DFLASH_IMAGE:-local/sglang-runtime:v0.5.15.post1-laguna-flashinfer0.6.15.post1-clean-experimental}"
EXP_IMAGE_ID="${DFLASH_IMAGE_ID:-sha256:50ce969dd51c2c95575e5417b4509dbca27289b38389b550839939b64875684f}"
...
BUNDLE_ID="${DFLASH_BUNDLE_ID:-laguna-s-2.1-nvfp4-dflash}"
SERVED_NAME="${DFLASH_SERVED_NAME:-laguna-s-2.1-agentic-experimental}"
EXPERIMENT_ROLE="${DFLASH_ROLE:-agentic-experimental}"
BUNDLE_SPEC="$PROJECT_ROOT/bundles/experimental/${BUNDLE_ID}/sglang.toml"
...
ACTUAL_IMAGE_ID="$(docker image inspect "$EXP_IMAGE" --format '{{.Id}}' 2>/dev/null || true)"
[ "$ACTUAL_IMAGE_ID" = "$EXP_IMAGE_ID" ] || {
  echo "REFUSING: experimental image mismatch for $EXP_IMAGE" >&2
  echo "expected=$EXP_IMAGE_ID actual=${ACTUAL_IMAGE_ID:-missing}" >&2
  exit 1
}
...
export DGX_RUNTIME_IMAGE="$EXP_IMAGE"
export DGX_RUNTIME_IMAGE_ID="$EXP_IMAGE_ID"
...
trap 'rm -f -- "$DGX_RUNTIME_CONFIG_DIR/sglang.yaml"; rmdir -- "$DGX_RUNTIME_CONFIG_DIR" 2>/dev/null || true' EXIT
# The bundle spec is an installed immutable artifact under PROJECT_ROOT.
SPEC_ARG="$BUNDLE_SPEC"
"$ADAPTER" "$EXPERIMENT_ROLE" "$RUNTIME_ROOT" "$PROJECT_ROOT" \
  "$BUNDLE_ID" "bundle" "$SPEC_ARG" \
  "$SERVED_NAME"
